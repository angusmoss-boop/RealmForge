/* Realmforge V11.21.0 — Canonical Finalisation Bundle */
...
/* Realmforge V11.21.0 — canonical system/UI ownership verification. */
(() => {
  'use strict';
  const RF=window.RF;
  ...
  const fieldcraft=RF.Systems?.Fieldcraft,gathering=RF.Systems?.Gathering,hunting=RF.Systems?.Hunting,fishing=RF.Systems?.Fishing,cooking=RF.Systems?.Cooking,world=RF.Systems?.World,social=RF.Systems?.Social;
  ...
  const focusClock=RF.Views?.FocusClock,database=RF.Views?.Database,navigation=RF.Views?.Navigation,presentation=RF.Views?.Presentation,overlays=RF.Views?.Overlays,appShell=RF.Views?.AppShell,developer=RF.Views?.Developer,lifecycle=RF.Core?.Lifecycle;
  const expected={
    ...
    world:["js/v2.js"],worldFragments:["v9_2-encounter-ecology"],socialFragments:["v4-social-core","v4-dialogue-ui","v4-people-ui","v4-guild-ui","v4-talk-action","v9_2-dialogue"],presentationFragments:["v9_1-weather-scene"]
  };
...
/* Realmforge V11.21.0 production finalizer. */
(() => {
  'use strict';
  const RF=window.RF;
  RF.VERSION='11.21.0';
  RF.BUILD={version:'11.21.0',title:'Canonical World & Social Foundations',built:'19 Sep 2026 • 19:16 BST',buildId:'20260919-v1121-world-social'};
  RF.PRODUCTION_FOUNDATION=RF.PRODUCTION_FOUNDATION||{};
