...
/* Realmforge V11.28.0 — canonical system/UI ownership verification. */
(() => {
  'use strict';
  const RF=window.RF;
  ...
  const focusClock=RF.Views?.FocusClock,database=RF.Views?.Database,navigation=RF.Views?.Navigation,presentation=RF.Views?.Presentation,overlays=RF.Views?.Overlays,appShell=RF.Views?.AppShell,developer=RF.Views?.Developer,itemBrowser=RF.Views?.ItemBrowser,worldActions=RF.Views?.WorldActions,lifecycle=RF.Core?.Lifecycle,scopedRuntime=RF.Core?.ScopedRuntime,migrations=RF.Core?.Migrations;
  const expected={
...
/* Realmforge V11.28.0 production finalizer. */
(() => {
  'use strict';
  const RF=window.RF;
  RF.VERSION='11.28.0';
  RF.BUILD={version:'11.28.0',title:'Scoped Residual Execution & Final Migration Bridges',built:'21 Sep 2026 • 09:20 BST',buildId:'20260921-v1128-scoped-residuals'};
  RF.PRODUCTION_FOUNDATION=RF.PRODUCTION_FOUNDATION||{};
  Object.assign(RF.PRODUCTION_FOUNDATION,{
    phase:21,architecture:'canonical-systems-v11',legacyBaseline:'11.5.3',compatibilityLayer:'js/legacy/compat_gameplay_scoped_residuals_trimmed_v1153.js',saveSchema:RF.Core.contract.saveSchema,
