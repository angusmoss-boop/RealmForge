...
/* Realmforge V11.29.0 — canonical system/UI ownership verification. */
(() => {
  'use strict';
  const RF=window.RF;
  ...
  const focusClock=RF.Views?.FocusClock,database=RF.Views?.Database,navigation=RF.Views?.Navigation,presentation=RF.Views?.Presentation,overlays=RF.Views?.Overlays,appShell=RF.Views?.AppShell,developer=RF.Views?.Developer,itemBrowser=RF.Views?.ItemBrowser,worldActions=RF.Views?.WorldActions,lifecycle=RF.Core?.Lifecycle,scopedRuntime=RF.Core?.ScopedRuntime,migrations=RF.Core?.Migrations,compatClassification=RF.Core?.CompatibilityClassification,authoring=RF.Authoring;
  const expected={
