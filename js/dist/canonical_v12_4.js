...
/* Realmforge V12.4.0 — canonical system/UI ownership verification. */
(() => {
  'use strict';
  const RF=window.RF;
  ...
  const focusClock=RF.Views?.FocusClock,database=RF.Views?.Database,navigation=RF.Views?.Navigation,presentation=RF.Views?.Presentation,overlays=RF.Views?.Overlays,appShell=RF.Views?.AppShell,saveManager=RF.Views?.SaveManager,developer=RF.Views?.Developer,itemBrowser=RF.Views?.ItemBrowser,itemDetail=RF.Views?.ItemDetail,worldActions=RF.Views?.WorldActions,lifecycle=RF.Core?.Lifecycle,scopedRuntime=RF.Core?.ScopedRuntime,migrations=RF.Core?.Migrations,compatClassification=RF.Core?.CompatibilityClassification,authoring=RF.Authoring,assets=RF.Assets;
  const expected={
...
/* Realmforge V12.4.0 production finalizer. */
(() => {
  'use strict';
  const RF=window.RF;
  RF.VERSION='12.4.0';
  RF.BUILD={version:'12.4.0',title:'Pack Dossiers & Tool Belt Integrity',built:'22 Sep 2026 • V12',buildId:'20260922-v1240-pack-dossiers'};
  RF.PRODUCTION_FOUNDATION=RF.PRODUCTION_FOUNDATION||{};
  Object.assign(RF.PRODUCTION_FOUNDATION,{
    phase:28,architecture:'canonical-systems-v12',legacyBaseline:'11.5.3',compatibilityLayer:'js/legacy/compat_gameplay_scoped_residuals_trimmed_v1153.js',saveSchema:RF.Core.contract.saveSchema,
