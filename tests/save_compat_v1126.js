...
checks.timeEnergyOwner=R.Modules?.info?.('systems.timeEnergy')?.meta?.status==='canonical';checks.focusClockOwner=R.Modules?.info?.('ui.focusClock')?.meta?.status==='canonical';checks.databaseOwner=R.Modules?.info?.('ui.database')?.meta?.status==='canonical'&&R.Views?.Database?.installedStages?.length===4;checks.navigationOwner=R.Modules?.info?.('ui.navigation')?.meta?.status==='canonical'&&R.Views?.Navigation?.installedStages?.length===1;checks.presentationOwner=R.Modules?.info?.('ui.presentation')?.meta?.status==='canonical'&&R.Views?.Presentation?.installedStages?.length===4;checks.overlaysOwner=R.Modules?.info?.('ui.overlays')?.meta?.status==='canonical'&&R.Views?.Overlays?.installedStages?.length===2;checks.appShellOwner=R.Modules?.info?.('ui.appShell')?.meta?.status==='canonical'&&R.Views?.AppShell?.installedStages?.length===1&&R.Views?.AppShell?.installedFragments?.length===1;checks.developerOwner=R.Modules?.info?.('ui.developer')?.meta?.status==='canonical'&&R.Views?.Developer?.installedStages?.length===1;checks.platformBridge=['copyText','promptText','alertMessage','vibrate','isVisible','onResume','fetchBuildInfo','registerServiceWorker','onBackNavigation','pushHistoryState','replaceHistoryState'].every(k=>typeof R.Platform?.active?.[k]==='function');checks.lifecycleOwner=R.Modules?.info?.('core.lifecycle')?.meta?.status==='canonical'&&R.Core?.Lifecycle?.booted===true;checks.worldOwner=R.Modules?.info?.('systems.world')?.meta?.status==='canonical'&&R.Systems?.World?.installedStages?.includes('js/v2.js')&&R.Systems?.World?.installedFragments?.includes('v7-mirefen-world-reveal');checks.socialOwner=R.Modules?.info?.('systems.social')?.meta?.status==='canonical'&&R.Systems?.Social?.installedFragments?.length===6;checks.encountersOwner=R.Modules?.info?.('systems.encounters')?.meta?.status==='canonical'&&R.Systems?.Encounters?.installedFragments?.length===3;checks.itemBrowserOwner=R.Modules?.info?.('ui.itemBrowser')?.meta?.status==='canonical'&&R.Views?.ItemBrowser?.installedFragments?.length===8;checks.characterOwner=R.Modules?.info?.('systems.character')?.meta?.status==='canonical'&&R.Systems?.Character?.installedFragments?.length===8;
checks.collectionOwner=R.Modules?.info?.('systems.collection')?.meta?.status==='canonical'&&R.Systems?.Collection?.installedFragments?.length===2;checks.characterShell=R.Systems?.Character?.installedFragments?.includes('v4-character-shell-ui');checks.propertyBuyHome=R.Systems?.Property?.installedFragments?.includes('v4-buy-home');
checks.inventoryAncestry=R.Systems?.Inventory?.installedStages?.includes('js/v10_10.js')&&R.Systems?.Inventory?.installedFragments?.length===9;
checks.equipmentAncestry=['v3-bulwark-armor','v3-equipment-ui','v8_2-equipment-foundation','v8_2-equipment-actions'].every(x=>R.Systems?.Equipment?.installedFragments?.includes(x));
checks.gatheringAncestry=['v8_2-mastery-work','v8_2-mastery-ui','v10-instant-harvest'].every(x=>R.Systems?.Gathering?.installedFragments?.includes(x));
checks.skillsAncestry=['v10-skills-ui','v10_12-skills-crafting-ui'].every(x=>R.Systems?.Skills?.installedFragments?.includes(x));
checks.combatResidual=['v3-combat-techniques','v3-combat-win-perks','v3-combat-ui','v8_2-combat-feedback','v8_3-cadence-motion','v9_1-parry-focus','v9_2-defeat-xp','v10-battle-summary','v4-combat-move-pools','v4-tactical-combat-core','v4-tactical-combat-ui'].every(x=>R.Systems?.Combat?.installedFragments?.includes(x));
checks.travelResidual=['v3-trailwise-travel','v8_3-travel-repair'].every(x=>R.Systems?.Travel?.installedFragments?.includes(x));
checks.developerAncestry=R.Views?.Developer?.installedFragments?.includes('v10-dev-energy');
checks.specialistOwner=R.Modules?.info?.('systems.specialist')?.meta?.status==='canonical'&&R.Systems?.Specialist?.installedFragments?.length===2;checks.worldActionsOwner=R.Modules?.info?.('ui.worldActions')?.meta?.status==='canonical'&&R.Views?.WorldActions?.installedFragments?.length===9;checks.v7v9Owners=R.Systems?.Locks?.installedFragments?.includes('v7-lockpicking')&&R.Systems?.Commerce?.installedFragments?.includes('v9-quantity-trade')&&R.Systems?.Research?.installedFragments?.includes('v7-bestiary-research')&&R.Systems?.Crafting?.installedFragments?.includes('v9-batch-crafting');checks.ownership=R.PRODUCTION_FOUNDATION?.systemOwnership?.valid===true;
