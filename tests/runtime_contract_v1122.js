(() => {
  ...
  ok(RF.VERSION==='11.22.0','version');
  ok(RF.Core?.contract?.appVersion==='11.22.0','core app version');
  ...
  ok(RF.Views?.ItemBrowser?.installedFragments?.length===4,'item browser fragments');
  ok(['v8_3-cadence-motion','v9_1-parry-focus','v9_2-defeat-xp'].every(x=>RF.Systems?.Combat?.installedFragments?.includes(x)),'combat residual fragments');
  ok(RF.Systems?.Travel?.installedFragments?.includes('v8_3-travel-repair'),'travel residual fragment');
  ...
  ok(['v9_3-developer-shell','v9_3-developer-actions-bind'].every(x=>RF.Views?.Developer?.installedFragments?.includes(x)),'developer ancestry fragments');
  ...
  window.RF_TEST_V1122={pass:fail.length===0,fail};
})();
