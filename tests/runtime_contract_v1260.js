(() => {
  ...
  ok(RF.VERSION==='12.6.0','version');
  ok(RF.Core?.contract?.appVersion==='12.6.0','core app version');
  ok(RF.Core?.contract?.saveSchema==='12.6.0','save schema');
  ...
  ok(RF.Modules?.info?.('systems.energyScale')?.meta?.status==='canonical','V12.6 Energy scale owner');
  ok(RF.Systems?.EnergyScale?.maxEnergy?.(1)===1000&&RF.Systems?.EnergyScale?.maxEnergy?.(4)===1060,'V12.6 Energy curve');
  ok(RF.Modules?.info?.('ui.workshop')?.meta?.status==='canonical'&&RF.Views?.Workshop?.columns===4&&RF.Views?.Workshop?.rows===2,'V12.6 modern workshop owner');
  window.RF_TEST_V1260={pass:fail.length===0,fail};
})();
