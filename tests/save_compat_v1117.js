...
const oldScripts=['js/data/base_content.js','js/legacy/base/state.js','js/legacy/base/ui.js','js/dist/save_core_v11_16.js','js/dist/data_core_v11_16.js','js/legacy/base/main.js','js/dist/systems_core_v11_16.js','js/legacy/compat_gameplay_exploration_crime_property_trimmed_v1153.js','js/dist/canonical_v11_16.js'];
const newScripts=['js/data/base_content.js','js/legacy/base/state.js','js/legacy/base/ui.js','js/dist/save_core_v11_17.js','js/dist/data_core_v11_17.js','js/legacy/base/main.js','js/dist/systems_core_v11_17.js','js/legacy/compat_gameplay_time_energy_trimmed_v1153.js','js/dist/canonical_v11_17.js'];
const clone=x=>JSON.parse(JSON.stringify(x)),same=(a,b)=>JSON.stringify(a)===JSON.stringify(b);
function baseAdvanced(R,name){
  let s=R.newGame(name,'traveller','🧭');R.state=s;s.player.level=30;s.player.xp=54321;s.gold=18888;s.location='greenvale';s.seed='v1117-compat-'+name;s.day=13;s.minute=16*60;s.player.maxEnergy=158;s.player.energy=67;s.speed=1;s.paused=false;
...
let f=bootFixture('Advanced', (R,s)=>{
  ...
  for(let i=0;i<42;i++){const id='compat17_dummy_'+i;R.DATA.items[id]={name:id,icon:'•',type:'material',value:1};s.inventory[id]=1;}
...
checks.timeEnergyOwner=R.Modules?.info?.('systems.timeEnergy')?.meta?.status==='canonical';checks.focusClockOwner=R.Modules?.info?.('ui.focusClock')?.meta?.status==='canonical';checks.ownership=R.PRODUCTION_FOUNDATION?.systemOwnership?.valid===true;
